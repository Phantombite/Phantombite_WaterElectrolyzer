...
{
    /// <summary>
    /// WaterElectrolyzer_Session — Core v2.0.0 Anbindung
    ///
    /// Kanal: 1995012 (Core → WaterElectrolyzer)
    ///
    /// Performance:
    ///   Level 0 — Normale Wasserproduktion (10 WaterFuel / 60 Ticks)
    ///   Level 3 — Produktion pausiert
    /// </summary>
    [MySessionComponentDescriptor(MyUpdateOrder.NoUpdate)]
    public class WaterElectrolyzer_Session : MySessionComponentBase
    {
        private const long   CORE_CHANNEL = 1995000L;
        private const long   MY_CHANNEL   = 1995012L;
        ...
        public override void LoadData()
        {
            ...
            {
                MyLog.Default.WriteLineAndConsole("[PB.WaterElectrolyzer] [ERROR] LoadData: " + ex);
            }
        }
        ...
        private void OnCoreMessage(object data)
        {
                ...
                if (msg.StartsWith("PERFLEVEL|"))
                {
                    ...
                    if (int.TryParse(msg.Substring(10), out lvl))
                    {
                        ...
                        Log("PERFLEVEL gesetzt: " + PerfLevel +
                            (PerfLevel >= 3 ? " — Produktion PAUSIERT" : " — Produktion AKTIV"));
                        MyAPIGateway.Utilities.SendModMessage(CORE_CHANNEL,
                            "PERFACK|waterelectrolyzer|" + PerfLevel);
                    }
                    return;
                }
            }
            ...
            {
                MyLog.Default.WriteLineAndConsole("[PB.WaterElectrolyzer] [ERROR] OnCoreMessage: " + ex);
            }
        }

        private void SendRegister()
        {
            string msg = "REGISTER|waterelectrolyzer|Water Electrolyzer|" + VERSION + "|" + MY_CHANNEL;
            MyAPIGateway.Utilities.SendModMessage(CORE_CHANNEL, msg);
            MyAPIGateway.Utilities.SendModMessage(CORE_CHANNEL, "PERFACK|waterelectrolyzer|0");
        }
        ...
        protected override void UnloadData()
        {
            ...
            {
                MyLog.Default.WriteLineAndConsole("[PB.WaterElectrolyzer] [ERROR] UnloadData: " + ex);
            }
        }
        ...
        private void Log(string msg, int level = 0)
        {
            ...
            try
            {
                ...
                MyAPIGateway.Utilities.SendModMessage(LOG_CHANNEL,
                    "LOG|" + MOD_NAME + "|" + level + "|WaterElectrolyzer_Session|" + msg);
            }
            catch { }
        }
    }
